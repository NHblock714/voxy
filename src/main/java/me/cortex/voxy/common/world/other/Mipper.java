...
public class Mipper {
    ...
    public static long mip(long I000, long I100, long I001, long I101,
        ...
        //Round the parent to the nearest state instead of always up - promoting a single solid child to
        //a full voxel biases every surface upward, which reads as a step at each lod ring. Exact halves
        //stay solid so one-block floors and roofs survive.
        int nonAir = 0;
        for (long s : states) {
            nonAir += Mapper.isAir(s) ? 0 : 1;
        }

        int dominantState = nonAir >= 4 ? chooseDominantState(states, mapper) : -1;
        if (dominantState != -1) {
            ...
            //Sky light takes the max, not the average: open-sky air is uniform, and averaging in zero-filled
            //children (sections with no stored sky light) dilutes surface air toward black over successive
            //mips. Fully dark air stays dark.
            int skyLight = Math.max(
                    Math.max(Math.max(Mapper.getLightId(I000) & 0x0F, Mapper.getLightId(I001) & 0x0F), Math.max(Mapper.getLightId(I010) & 0x0F, Mapper.getLightId(I011) & 0x0F)),
                    Math.max(Math.max(Mapper.getLightId(I100) & 0x0F, Mapper.getLightId(I101) & 0x0F), Math.max(Mapper.getLightId(I110) & 0x0F, Mapper.getLightId(I111) & 0x0F)));
            blockLight = (blockLight / 8) & 0xF0;

            //When the majority vote rejected a minority of solid children (nonAir > 0), the parent
            //must be explicit air - I111 is an arbitrary child and may be one of the solids.
